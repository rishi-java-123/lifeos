...
function App(){
 ...
 return <div className="app">
  ...
  {tab==='inventory'&&<Inventory data={data} setData={setData} setModal={setModal}/>} 
...
function Inventory({data,setData,setModal}){return <main><Top title="Home Inventory"/><div className="searchbox"><Search size={18}/><input placeholder="Search items..."/></div><div className="stats"><Stat n={data.items.length} t="Total Items"/><Stat n="8" t="Expiring Soon"/><Stat n="5" t="Low Stock"/></div><div className="sectionTitle"><b>Recently Added</b><span>View all</span></div><Card>{data.items.map(x=><div className="row" key={x.id}><Package size={22}/><div><b>{x.name}</b><small>{x.cat} · {x.room}</small></div><button className="iconBtn" onClick={()=>setData(d=>({...d,items:d.items.filter(i=>i.id!==x.id)}))}><Trash2 size={16}/></button></div>)}</Card><button className="primary wide" onClick={()=>setModal('item')}>＋ Add Item</button></main>}
function Medicines({data,toggleMed}){return <main><Top title="Medicines"/><div className="datebar">Sun　Mon　 <b>Tue</b>　Wed　Thu　Fri　Sat</div><h3>Today's Schedule</h3><Card>{data.meds.map(x=><div className="row" key={x.id}><Pill size={20}/><div><b>{x.name}</b><small>{x.dose}</small></div><button className={x.taken?'pill done':'pill'} onClick={()=>toggleMed(x.id)}>{x.taken?'Taken':'Mark'}</button></div>)}</Card></main>}
function Bills({data}){return <main><Top title="Bills & Subscriptions"/><Card cls="total"><small>Total Due</small><strong>{money(data.bills.reduce((s,x)=>s+x.amount,0))}</strong><span>This Month</span></Card>{data.bills.map(x=><Card key={x.id} cls="bill"><div><b>{x.name}</b><small>{x.provider} · Due {x.due}</small></div><strong>{money(x.amount)}</strong></Card>)}</main>}
function Expenses({data,total,setModal}){return <main><Top title="Expenses"/><Card cls="total"><small>Total Expenses</small><strong>{money(total)}</strong><span>Local data · this device</span></Card><Card><h3>Recent Transactions</h3>{data.expenses.map(x=><div className="row" key={x.id}><Receipt size={20}/><div><b>{x.name}</b><small>{x.cat}</small></div><strong>{money(x.amount)}</strong></div>)}</Card><button className="primary wide" onClick={()=>setModal('expense')}>＋ Add Expense</button></main>}
function Reminders({data}){return <main><Top title="Reminders"/>{data.tasks.map(x=><Card key={x.id} cls="bill"><div><b>{x.title}</b><small>{x.date}</small></div><ChevronRight/></Card>)}</main>}
function Assistant({data}){const [q,setQ]=useState('Where is my passport?');const answer=q.toLowerCase().includes('passport')?'I can search your local LifeOS inventory. In this demo, passport is not stored yet. Add it under Home Inventory or Documents.':q.toLowerCase().includes('expense')?`Your local expenses total ${money(data.expenses.reduce((s,x)=>s+x.amount,0))}.`:'I can help with your tasks, inventory, bills, medicines and expenses stored on this device.';return <main><Top title="AI Assistant"/><div className="chat"><div className="bubble user">{q}</div><div className="bubble">{answer}</div><div className="suggestions"><button onClick={()=>setQ('Where is my passport?')}>Where is my passport?</button><button onClick={()=>setQ('Show my expenses')}>Show my expenses</button><button onClick={()=>setQ('What reminders do I have?')}>Show reminders</button></div></div></main>}
...
function Profile({reset}){return <main><Top title="Profile"/><Card><div className="profile"><div className="avatar">R</div><div><h2>My LifeOS</h2><p>Personal workspace · offline</p></div></div></Card><Card><h3>Data</h3><p>All demo data is saved in your browser using localStorage. No backend or account is required.</p><button className="danger" onClick={reset}>Reset Demo Data</button></Card></main>}
const Top=({title})=><div className="top"><button onClick={()=>history.back()}><span>‹</span></button><h2>{title}</h2></div>;
