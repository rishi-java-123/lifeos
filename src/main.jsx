...
const seed={
 family:[{id:1,name:'Rishi',relationship:'Self',dob:'',phone:''}],
...
function App(){
 ...
 return <div className="app">
  ...
  {tab==='family'&&<Family data={data} setData={setData} setModal={setModal}/>} 
...
function HomePage({data,setTab,toggleTask,setModal}){return <main>
 ...
 <div className="sectionTitle"><b>Quick Access</b><span>Edit</span></div><div className="quick">{[[Folder,'Documents','docs'],[Pill,'Medicines','medicines'],[Wallet,'Expenses','expenses'],[Bell,'Reminders','reminders'],[Home,'Home Inventory','inventory'],[Receipt,'Bills & Subscriptions','bills'],[ShoppingCart,'Shopping List',''],[Users,'Family','family']].map(([I,t,id])=><button key={t} onClick={()=>id&&setTab(id)}><I size={24}/><span>{t}</span></button>)}</div>
...
function Family({data,setData,setModal}){const members=data.family||[];return <main><Top title="Family" onBack={()=>window.dispatchEvent(new Event('lifeos-home'))}/><Card cls="familyHero"><div><small>Family Members</small><strong>{members.length}</strong><span>people in your LifeOS</span></div><Users size={42}/></Card><div className="sectionTitle"><b>Members</b><span>{members.length} total</span></div>{members.map(x=><Card key={x.id} cls="memberCard"><div className="memberAvatar">{x.name.charAt(0).toUpperCase()}</div><div className="memberInfo"><b>{x.name}</b><small>{x.relationship}{x.dob?' · '+x.dob:''}</small>{x.phone&&<small>{x.phone}</small>}</div>{x.relationship!=='Self'&&<button className="iconBtn" onClick={()=>setData(d=>({...d,family:(d.family||[]).filter(m=>m.id!==x.id)}))}><Trash2 size={16}/></button>}</Card>)}<button className="primary wide" onClick={()=>setModal('family')}>＋ Add Family Member</button><Card cls="familyFeatures"><h3>Family LifeOS</h3><p>Once members are added, you can associate medicines, bills, reminders, expenses and documents with each person.</p></Card></main>}
